...
---
# GIAI ĐOẠN 2 — Tái cấu trúc chuyên nghiệp & dọn code (cùng ngày)
- **Cấu trúc mới:** lõi upstream giữ phẳng ở root; PIVOT tách vào `experiments/` (8 script chạy thí nghiệm) và `analysis/` (4 công cụ tổng hợp); `docs/PIVOT.pdf`; `grid_wn18rr_master.log` → `reports/artifacts/logs/`. Chi tiết + lý do: **CODE_AUDIT.md**.
- **Dọn code:** cắt ~520 dòng chết/trùng (learned_pruning 304→40; xóa git_push_update; gộp 2 sweep thành 1 với `--fact_ratio`; bỏ import trùng; bỏ bản sao get_hop_distances). Không đổi hành vi tính toán.
- **Đường dẫn nhất quán:** scripts ghi kết quả vào `reports/grid_t78_*` / `reports/robustness_t10` (đúng nơi minh chứng đang nằm, resume-safe); mọi lệnh trong README + walkthrough cập nhật theo layout mới; quy ước gọi từ repo root.
- **Cách áp dụng:** `bash apply_restructure.sh` (git mv giữ lịch sử) → `unzip -o PIVOT_pro_restructure.zip` → `git add -A && git commit -m "refactor: professional layout (experiments/analysis), remove dead code, unify paths" && git push`.
