...
![Pareto Frontier WN18RR](file:///home/vanba/KLTN/one-shot-subgraph/grid_t78_wn18rr/figure1_frontier_wn18rr.png)
...
![Pareto Frontier NELL-995](/home/vanba/KLTN/one-shot-subgraph/grid_t78_nell/figure1_frontier_nell.png)
...
![Pareto Frontier NELL-995](file:///home/vanba/KLTN/one-shot-subgraph/grid_t78_nell/figure1_frontier_nell.png)

##### 5. Demo BudgetController trên NELL-995

* **Truy vấn 1: "Tìm cấu hình MRR tốt nhất dưới ràng buộc Latency ≤ 30 ms"** *(ràng buộc cắt vào frontier)*
  - Lệnh truy vấn: `python3 pareto_optimizer.py --cache_path grid_t78_nell/pareto_cache_nell_v2.json --max_latency 30.0`
  - Trả về: **PIVOT-Rerank @ θ=5%** → Valid MRR: `0.5655`, Test MRR: `0.5468 ± 0.0042`, Latency/q: `27.77 ms`, VRAM: `1715 MB`.
* **Truy vấn 2: "Tìm cấu hình Latency thấp nhất để đạt MRR ≥ 0.568"** *(ràng buộc cắt vào frontier)*
  - Lệnh truy vấn: `python3 pareto_optimizer.py --cache_path grid_t78_nell/pareto_cache_nell_v2.json --min_mrr 0.568`
  - Trả về: **PIVOT-Rerank @ θ=10%** → Valid MRR: `0.5684`, Test MRR: `0.5448 ± 0.0038`, Latency/q: `41.78 ms`, VRAM: `2547 MB`.

Hai truy vấn trả về hai điểm **khác nhau** trên frontier (θ=5% và θ=10%) — minh họa đúng tinh thần trade-off của BudgetController trên tập NELL-995.

> **Lưu ý:** Tại truy vấn thứ hai, cấu hình được chọn theo Valid (θ=10%, Valid `0.5684`) có **Test MRR thấp hơn** cấu hình θ=5% (Test `0.5448` < `0.5468`) — minh họa cụ thể cho limitation Valid/Test gap trên NELL đã nêu ở trên. Controller trung thành với protocol **chọn-theo-Valid** và không nhìn vào Test khi quyết định; sự nghịch lý trong không gian Test là hệ quả trực tiếp của hiện tượng double-dipping vào tập valid 543 query.

##### 6. Nhận xét và phân tích chuyên sâu chéo

Phân tích Pareto trên NELL-995 tiếp tục khẳng định hiệu quả vượt trội của PIVOT-Rerank so với baseline PPR-only. Ở các mức budget siêu nhỏ như θ=1%, PIVOT-Rerank cải thiện Valid MRR từ 0.4840 lên 0.5357 và Test MRR từ 0.5361 lên 0.5414, mang lại mức cải thiện đáng kể với chi phí latency tăng không đáng kể (15.17 ms/q vs 6.84 ms/q). Ở budget θ=5%, PIVOT-Rerank thiết lập cấu hình tối ưu Pareto nổi bật nhất với Test MRR vượt trội đạt 0.5468 và latency rất thấp là 27.77 ms/q, tối ưu hóa đáng kể so với việc chạy full graph. Điều này khẳng định tính ổn định của cơ chế reranking dựa trên MLP đối với các tập dữ liệu có quy mô lớn và đồ thị dày đặc như NELL-995.

> ⚠️ **Limitation — Valid/Test gap trên NELL:** Tại θ=10%, PIVOT-Rerank đẩy Valid MRR từ 0.5003 lên 0.5684 (+0.0681) nhưng Test MRR chỉ tăng từ 0.5354 lên 0.5448 (+0.0094). Chênh lệch này **không phải lỗi grid** (khớp đúng sweep Tuần 9) mà phản ánh hiện tượng "double dipping" vào tập valid: tập validation NELL-995 chỉ gồm 543 query và đã được dùng để (i) early-stop training MLP pruning và (ii) chọn α*. Do đó mức cải thiện valid bị phóng đại bởi overfit nhẹ, trong khi Test MRR — tập không bị đụng trong toàn bộ pipeline — là con số trung thực. **Mọi claim hiệu năng trong §7–9 đều dựa trên cột Test MRR.**

**Hybrid NELL-995:** [CHƯA THỰC HIỆN — tùy chọn] Theo thiết kế, chiến dịch NELL-995 chạy PPR-only và PIVOT-Rerank (48 lượt). Để bổ dung Hybrid+Rerank: `INCLUDE_HYBRID=1 bash run_grid_t78.sh nell` (24 lượt bổ sung, resume-safe). Kết quả WN18RR gợi ý Hybrid thắng ở θ=1% (VRAM thấp hơn 27% so với PPR/Rerank) và θ=20%; finding này có thể lặp trên NELL do cùng cơ chế MLP-filtering giảm candidate pool trước GNN.
...
![Tier-1 and Tier-2 Feature Ablation](/home/vanba/KLTN/one-shot-subgraph/reports/figures/figure_feature_ablation.png)
...
![Degradation Curves](/home/vanba/KLTN/one-shot-subgraph/reports/figures/figure_robustness_wn18rr.png)
...
### Nhận Xét Số Liệu (Kịch Bản Cải Thiện Bền Vững)
Dựa trên bảng tổng hợp `robustness_agg.csv`, kết quả hoàn toàn ủng hộ kịch bản **(b) Hai đường song song → cải thiện bền vững dưới nhiễu**. Khi mức độ xóa cạnh ngẫu nhiên tăng dần từ 0% (clean) lên 20% (del20), mặc dù độ chính xác của cả hai phương pháp đều suy giảm, đường Rerank degrade cùng tốc độ với đường PPR (song song). Đặc biệt, mức độ cải thiện tuyệt đối của Rerank so với PPR luôn **giữ vững** ổn định ở mức `+0.003` đến `+0.005` (tại clean là +0.0047, tại del20 là +0.0043). 

Tại kịch bản khắc nghiệt nhất là **reldel** (nhắm trực tiếp vào việc xóa 50% số cạnh của 30% các relation hiếm nhất), Test MRR của PPR-only giảm từ 0.5638 xuống 0.5616, trong khi PIVOT-Rerank đạt 0.5663, duy trì khoảng cách chênh lệch `+0.0047`. Khả năng duy trì biên độ khoảng cách này khẳng định rằng module MLP learned feature không hề bị catastrophic distribution shift khi đồ thị bị khuyết cạnh. Tính chống chịu lỗi cục bộ của PIVOT-Rerank hoạt động cực kỳ ổn định.
...
* **Giải pháp:** Kết hợp song song hai giải pháp:
  - **Bổ sung giải phóng bộ đệm VRAM:** Thêm `torch.cuda.empty_cache()` vào cuối mỗi batch huấn luyện trong [base_model.py](file:///home/vanba/KLTN/one-shot-subgraph/base_model.py). Việc này ép PyTorch trả lại bộ nhớ GPU ảo ngay lập tức cho dxgkrnl, ngăn ngừa hiện tượng tích lũy bộ đệm gây sập WSL.
