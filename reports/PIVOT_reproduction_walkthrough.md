...
| Chỉ số | Checkpoint gốc tác giả | PIVOT Baseline (PPR-only)* | PIVOT Reranking (α=0.82) | So với Gốc |
|:---|:---:|:---:|:---:|:---:|
| **Test MRR** | 0.5677 | 0.5644 | **0.5699** | **+0.0022** (Vượt trội) |
| Test H@1 | 51.40% | 51.18% | **52.01%** | **+0.61%** |
| Test H@10 | 66.62% | 66.34% | **67.12%** | **+0.50%** |
| **Inference Time (s)** | 439.62s | **157.7s** | **233.1s** | **~1.9x Nhanh hơn** ⚡ |
...
1. **Chất lượng lập luận vượt trội:** Bằng việc tích hợp tri thức ngữ nghĩa từ mô hình MLP Pruning ở giai đoạn xếp hạng cuối (Post-hoc Reranking), PIVOT với $\alpha=0.82$ đạt MRR = **0.5699**, chính thức vượt qua checkpoint tốt nhất được công bố của tác giả gốc (**0.5677**).
2. **Tốc độ vượt trội (gấp ~1.9x đến 2.7x):** Baseline PPR của chúng ta chỉ tốn **157.7 giây** (nhanh hơn **2.7x** so với 439.62 giây của tác giả). Khi bật tính năng Reranking, tổng thời gian inference vẫn chỉ mất **233.1 giây** (nhanh hơn **1.9x**). Sự bứt phá này đạt được nhờ cơ chế **Pre-loading PPR Cache** lên bộ nhớ trong toàn cục và xử lý song song, triệt tiêu hoàn toàn nghẽn cổ chai disk I/O của tác giả.
